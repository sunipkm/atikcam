...
def animate(i):
    ...
    fig.title(r"CoMIC Instrument Monitor: Frame %d"%(i))
    fig.suptitle(r"Timestamp: %s, exposure: %.3f s\nCCD Temperature: %.2f $^\circ$, Instrument Temperature: %.2f $^\circ$"%(datetime.datetime.fromtimestamp(a.tnow/1e3),a.exposure,a.ccdtemp/100,a.bodytemp/100))
